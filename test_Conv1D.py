...
parser.add_argument('--env', default='snuh', help='Test environment snuh or open')
parser.add_argument('--tflite', action='store_true', help='Use tflite for test')
...
if FLAGS.tflite:
    ...
    if model_flag == 0:
        ...
        model = nets.Conv1D_SPP(dil_fac=dil_fac)
    elif model_flag == 7:
        dil_fac = dim_HT1D[0] // 3000
...
if ENVIRON == 'snuh':
    ...
    PROCESSED_DATA_PATH = os.path.join('/tf','00_data','kdpark', 'origin_npy') 
    ...
    if include_SC:
        list_files_test += list_files_SC

        for f in list_files_SC:
    ...
    if include_ST:       
        ...
        list_files_test += list_files_ST
        for f in list_files_ST:
...
if not FLAGS.tflite:
    LOG_FOUT = open(os.path.join(LOG_DIR, 'test_conv1d_' + model.name + '_' + log_fname + '.txt'), 'a')
else:
    LOG_FOUT = open(os.path.join(LOG_DIR, 'test_conv1d_' + tflite_name[:-7] + '_' + log_fname + '.txt'), 'a')

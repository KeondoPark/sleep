...
if model_flag == 0:
    ...
    model = nets.Conv1DASPP_single(dil_fac=dil_fac)
    model2 = nets.Conv1DASPP_multi_lstm(batch_size=bs, prev_cnt=PREV_CNT)


x = np.random.random((bs,) + dim_HT1D)
x = tf.convert_to_tensor(x)
x2 = np.random.random((bs,PREV_CNT+1,) + dim_HT1D)
...
if ENVIRON == 'snuh':
    ...
    save_signals_path_seq = os.path.join(PROCESSED_DATA_PATH,'signals_seq')
    save_annotations_path_seq = os.path.join(PROCESSED_DATA_PATH,'annotations_seq')
    
    list_files = [os.path.join(save_signals_path, f) for f in os.listdir(save_signals_path) if f.endswith('.npy')]
    list_files_seq = [os.path.join(save_signals_path_seq, f) for f in os.listdir(save_signals_path_seq) if f.endswith('.npy')]
    ...
    for f in list_files_train:
        ...
        #Sequence files
        list_seq_files_train.append(os.path.join(save_signals_path_seq, os.path.basename(f)))
        list_ann_seq_files_train.append(os.path.join(save_annotations_path_seq, ann_file[0]))
    ...
    for f in list_files[split_cnt:]:
        ...
        #Sequence files
        list_seq_files_test.append(os.path.join(save_signals_path_seq, os.path.basename(f)))
        list_ann_seq_files_test.append(os.path.join(save_annotations_path_seq, ann_file[0]))
